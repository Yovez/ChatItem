...
public class ChatItem extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (isMc112Orlater()) {
			...
			try {
				chatMessageTypeClass = Class
						.forName("net.minecraft.server." + getVersion(Bukkit.getServer()) + ".ChatMessageType");
			} catch (ClassNotFoundException e) {
				e.printStackTrace(); // This should never happen anyways, so no need to think of fancy stuff like
		...
		// We halt the use of this system for now, until we can solve the infamous
		// getProtocolVersion issue
		// Till then, users of both ViaVersion and ProtocolSupport should do just fine
		/*
		 * if(!protocolSupport && !viaVersion) { //We only implement our own way of
		 * getting protocol versions if we have no other choice pm.addPacketListener(new
		 * HandshakeListener(this, ListenerPriority.MONITOR,
	...
	public static JSONManipulator getManipulator() {
		/*
		 * We used to have 2 kinds of JSONManipulators because of my bad understanding
		 * of the 1.7 way of parsing JSON chat The interface should however stay as
		 * there might be great changes in future versions in JSON parsing (most likely
		 * 1.13)
		 */
		return new JSONManipulatorCurrent();
		// We just return a new one whenever requested for the moment, should implement
		// a cache of some sort some time though
	}
